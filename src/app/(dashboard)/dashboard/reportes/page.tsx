    ...
    <div style={{ maxWidth: 1100 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14, marginBottom: 28 }}>
        {[
          ...
          { label: "Ingresos del año",   value: formatCurrency(yearTotal), sub: `Año ${year}`, color: "#1d4ed8" },
              ...
              <div key={m.month} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
                ...
                <div style={{ width: "100%", display: "flex", alignItems: "flex-end", height: 120 }}>
                  <div style={{
                    ...
                    background: isCurrent ? "#1e3a5f" : "#bfdbfe",
                    borderRadius: "4px 4px 0 0",
                    minHeight: m.total > 0 ? 4 : 0,
                    transition: "height 0.3s",
                  }} />
                </div>
                <span style={{ fontSize: 10, color: isCurrent ? "#1e3a5f" : "var(--color-text-secondary)", fontWeight: isCurrent ? 600 : 400 }}>
                  {MONTHS[m.month]}
                </span>
              </div>
            );
            ...
            <tbody>
              {monthlyRevenue.slice(0, month + 1).reverse().map(m => (
                <tr key={m.month} style={{ borderBottom: "0.5px solid var(--color-border-tertiary)" }}>
                  <td style={{ padding: "7px 8px", color: m.month === month ? "#1e3a5f" : "var(--color-text-primary)", fontWeight: m.month === month ? 600 : 400 }}>
                    {MONTHS[m.month]}
                  </td>
