            ...
            {isLoading ? (
              ...
              <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16, flex: 1, overflow: "hidden" }}>
                ...
                <div style={{ display: "flex", justifyContent: "flex-end", gap: 10, paddingTop: 10, borderTop: "0.5px solid var(--color-border-tertiary)", marginTop: "auto" }}>
                  ...
                  <button type="submit" disabled={!groupId || transfer.isLoading} style={{ padding: "8px 16px", borderRadius: 8, border: "none", background: "#1e3a5f", color: "#fff", cursor: (!groupId || transfer.isLoading) ? "not-allowed" : "pointer", opacity: (!groupId || transfer.isLoading) ? 0.7 : 1 }}>
